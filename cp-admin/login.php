...
if (!isset($_SESSION['type']) || $_SESSION['type'] !== 'admin') {
    ...
    if ($_SERVER['REQUEST_METHOD'] == "POST" && isset($_POST['submit'])) {
        ...
        $selectAdmin = "SELECT * FROM workers WHERE username = '$username' and password = '" . hash('sha256', $password) . "'";
        $execute = mysqli_query($conn, $selectAdmin);
        $rows = mysqli_fetch_assoc($execute);


        if ($rows) {
            $_SESSION['idUWorker'] = $rows['id'];
