...
if (!isset($_SESSION['type']) || $_SESSION['type'] !== 'worker') {
    ...
    // SELECT FULLNAME 
    $querySelectName =  "SELECT booking.id_booking, register.fullname, booking.service, booking.numberPhone, booking.address, booking.date, booking.time, booking.status
    FROM booking
...
<body>
    ...
    <div class="container">
                    ...
                    ?>
                            <tr>
                                ...
                                <td>
                                    ...
                                    <?php {
                                                if ($runs["id_booking"] == $row["id_booking"]) {
                                                    echo $runs["fullname"];
                                                    break;
                                                }
                                            }
                               ...
                               <?php if ($row['typeOfService'] == 'yes') {
                                ...
                                ?>
                                <td class='action'>
                                    <a class='edit' href='edit__booking.php?id=<?= $row['id_booking']; ?>'><i class="fas fa-edit"></i></a>
                                    <a class='delete' href='delete__booking.php?id=<?= $row['id_booking']; ?>'><i class="fa-solid fa-trash-can"></i></a>
                                </td>
        ...
        <div class="pagination">
            <?php if ($page > 1) {
                echo "<a href='index.php?page=booking&page_number=" . ($page - 1) . "' class='arrowLeft'><i class='fa-solid fa-chevron-left'></i></a>";
            } ?>
            ...
            for ($i = 1; $i < $total_pages; $i++) {
                echo '<a href="index.php?page=booking&page_number=' . $i . '" class="numPages">' . $i . '</a>';
            }
            ?>

            <?php if ($page < $total_pages) {
                echo "<a href='index.php?page=booking&page_number=" . ($page + 1) . "' class='arrowRight'><i class='fa-solid fa-chevron-right'></i></a>";
            } ?>
